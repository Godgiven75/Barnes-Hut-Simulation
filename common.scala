...
case class Empty(centerX: Float, centerY: Float, size: Float) extends Quad:
  ...
  def mass: Float = 0f // Zero mass because empty <=> no bodies in the cell
  def total: Int = 0 // No bodies inside the cell
...
extends Quad:
  val (mass, massX, massY) =
    (bodies.foldLeft(0f)(_ + _.mass) : Float, // très mauvaise idée d'initialiser simultanément par un triplet, ça ne permet pas de recycler "mass"...
    bodies.foldLeft(0f) {case (acc, b) => acc + b.mass * b.x} / bodies.foldLeft(0f)(_ + _.mass) : Float,
    bodies.foldLeft(0f) {case (acc, b) => acc + b.mass * b.y} / bodies.foldLeft(0f)(_ + _.mass) : Float)
  ...
  def insert(b: Body): Quad =
    if (size > minimumSize) {
      ...
      val newFork = Fork(Empty(centerX - quarterSize, centerY - quarterSize, halfSize),
        ...
        Empty(centerX - quarterSize, centerY + quarterSize, halfSize),
        Empty(centerX + quarterSize, centerY + quarterSize, halfSize))
      (b +: bodies).foldLeft(newFork)((f, b) => f.insert(b)) // Inserting all the bodies in the new Fork
    }
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    def posToIdx(p1: Float, p2: Float): Int =
      val tmp = ((p2 - p1) / sectorSize).toInt // Transformation vis-à-vis de l'origine : translation puis downscale sur (0, 0)
      val downClamp = Math.max(tmp, 0) // Clamp dans le cas d'un body en dehors des bounadries
      Math.min(downClamp, sectorPrecision - 1)
    this(posToIdx(boundaries.minX, b.x), posToIdx(boundaries.minY, b.y)) += b
    this
  ...
  def combine(that: SectorMatrix): SectorMatrix =
    ...
    for i <- 0 until matrix.length do newSectorMatrix.matrix(i) = matrix(i).combine(that.matrix(i))
    newSectorMatrix
